...
def enviar_alerta_telegram(action, pnl, sl):
    # Formato de mensaje profesional y bonito directo para tus miembros
    mensaje_oficial = (
        ...
        f"🎯 *TARGET ACONSEJADO:* +{pnl} PNL (Cierre rápido)\n"
        f"🛑 *STOP LOSS PROTEGIDO:* -{sl} PNL (Detrás de acumulación)\n\n"
        f"¡Revisa tu app de Bitget y asegura tu posición! 💥"
    )
    
    # Conexión nativa con la API oficial de Telegram
    url = f"https://telegram.org{TOKEN_TELEGRAM}/sendMessage"
    payload = {
        "chat_id": ID_CHAT_CANAL,
        "text": mensaje_oficial,
        "parse_mode": "Markdown" # Para que los textos salgan en negrita bonitos
    }
